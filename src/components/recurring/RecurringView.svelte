...
<div class="px-4 sm:px-6 py-6">
  ...
  {:else}
    ...
    <div class="grid grid-cols-1 gap-4">
      {#each recurringExpenses.filter((item) => item.frequencyDays <= 31) as recurringExpense (recurringExpense.id)}
        {@render expenseCard(recurringExpense)}
      {/each}
    </div>
    <div class="grid grid-cols-1 gap-4 mt-8">
      <h2 class="text-sm tracking-wider uppercase font-semibold">Less Frequent</h2>
      {#each recurringExpenses.filter((item) => item.frequencyDays > 31) as recurringExpense (recurringExpense.id)}
        {@render expenseCard(recurringExpense)}
      {/each}
    </div>
